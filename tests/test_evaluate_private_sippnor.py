...
def test_openModel():
    filename = os.path.dirname(os.path.abspath(__file__)) + "/pyplan-private-models/SIPPNOR/sippnor.ppl"
...
def test_evaluateNode():
    ...
    print(value)
    assert value == 5843893567.538011, "Error on evaluate node. The node result is " + str(value)
