...
def test_clone_git_repo():
    ...
    os.system("cd pyplan-private-models && git fetch && git pull")

def test_openModel():
    filename = os.path.dirname(os.path.abspath(__file__)) + "/pyplan-private-models/SIPME/sipme.ppl"
...
def test_evaluateNode():
    ...
    print(value)
    assert value == 70827042042.76224, "Error on evaluate node. The node result is " + str(value)
